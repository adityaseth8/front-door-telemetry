...
int build_json(float x, float y, float z, char *json, size_t json_size) {
    return snprintf(json, json_size,
                    "{\"x\": %.3f, \"y\": %.3f, \"z\": %.3f}",
                    x, y, z);
}

void prepare_and_send_data(float x, float y, float z) {

    // Build JSON string
    char json[128];
    build_json(x, y, z, json, sizeof(json));
    printf("JSON: %s\n", json);
...
int main() {
    ...
    while (true) {
        ...
        prepare_and_send_data(d.x, d.y, d.z);
